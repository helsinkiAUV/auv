...
boolean Adafruit_GPS::waitForSentence(const char *wait4me, uint8_t max) {
  ...
  //Serial.println("waitForSentence");

  uint8_t i=0;
  while (i < max) {
    //Serial.println("waitForSentence->while");
    ...
    //Serial.println(read());
    if (newNMEAreceived()) { 
      //Serial.println("waitForSentence->while->if");
      char *nmea = lastNMEA();
      strncpy(str, nmea, 20);
      str[19] = 0;
      i++;
      ...
      if (strstr(str, wait4me)) {
        //Serial.println("Comparing string and wait4me");
        i = max + 100;
        return true;
      }
	//return true;
